  ...
  return (
    <>
      <div className="flex w-full items-center gap-2">
        ...
        <button onClick={onClear} disabled={isActing || plannedActions.length === 0} className="rounded-lg border border-gray-600 px-3 py-2 text-sm text-gray-300 disabled:cursor-not-allowed disabled:opacity-40">
          Clear
        </button>
      </div>
      {attacks.length > 0 && (
      ...
      <button onClick={onEndTurn} disabled={!isMyTurn || isEndingTurn} className={`w-full py-2.5 rounded-lg font-semibold transition ${isMyTurn ? "bg-yellow-600 hover:bg-yellow-500 text-white" : "bg-gray-700 text-gray-400 cursor-not-allowed"}`}>
        End Turn
      </button>
      <button onClick={onFinishCombat} disabled={!isMyTurn || isActing || combat.status !== "IN_PROGRESS"} className="w-full rounded-lg border border-red-700 py-2 text-sm text-red-300 hover:bg-red-950 disabled:cursor-not-allowed disabled:opacity-40">
